...
function DocDetail({doc,onClose,onUpdate,connected}){
  ...
  const uploadFiles=async(fileList)=>{
    ...
    for(const file of fileList){
      ...
      const storagePath=`${doc.id}/${ts}_${safeName}`;
      ...
      const row={document_id:doc.id,file_name:file.name,file_path:storagePath,file_size:file.size,file_type:file.type||file.name.split('.').pop(),version:fileVer,uploader:'사용자',description:fileDesc,is_latest:true,public_url:publicUrl};
      ...
      const{data:inserted}=await supabase.from('file_attachments').insert(row).select();
  ...
  const downloadFile=async(f)=>{
    ...
    const a=document.createElement('a');a.href=url;a.download=f.file_name;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
  };

  const deleteFile=async(f)=>{
    if(!confirm(`"${f.file_name}" 파일을 삭제하시겠습니까?`))return;
    if(supabase&&connected){
      await supabase.storage.from('gmp-files').remove([f.file_path]);
      await supabase.from('file_attachments').delete().eq('id',f.id);
    }
...
/* ══════════════════════════════════════════════════
   ISSUES LIST
   ══════════════════════════════════════════════════ */
function IssueList({issues,onUpdate}){
  const[editId,setEditId]=useState(null);
  return<div>
    <div className="section-title">⚠️ 이슈 관리 <span style={{fontSize:'11px',color:'var(--g4)',fontWeight:400}}>({issues.length}건)</span></div>
    <div className="tbl-wrap"><table className="tbl">
      <thead><tr><th>ID</th><th>제목</th><th>심각도</th><th>상태</th><th>마감일</th><th>의사결정</th><th>설명</th></tr></thead>
      <tbody>{issues.map(i=><tr key={i.id}onClick={()=>setEditId(i.id)}>
        <td style={{fontFamily:'IBM Plex Mono',fontSize:'11px'}}>{i.id}</td>
...
function IssueEditModal({issue,onClose,onUpdate}){
  ...
  return<div className="modal-overlay"onClick={onClose}><div className="modal"onClick={e=>e.stopPropagation()}>
    ...
    <div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:16}}>
      <button className="btn btn-ghost"onClick={onClose}>취소</button>
      <button className="btn btn-primary"onClick={()=>{onUpdate(form);onClose();}}>저장</button>
    </div>
  </div></div>;
}
...
function RiskList({risks}){
  return<div>
    <div className="section-title">🛡️ 리스크 관리 <span style={{fontSize:'11px',color:'var(--g4)',fontWeight:400}}>({risks.length}건)</span></div>
    <div className="grid-2">{risks.map(r=><div key={r.id}className="card"style={{position:'relative',overflow:'hidden'}}>
...
/* ══════════════════════════════════════════════════
   ACTION ITEMS
   ══════════════════════════════════════════════════ */
function ActionList({actions,onUpdate}){
  return<div>
    <div className="section-title">🎯 Action Items <span style={{fontSize:'11px',color:'var(--g4)',fontWeight:400}}>({actions.length}건)</span></div>
    <div className="tbl-wrap"><table className="tbl">
      <thead><tr><th>ID</th><th>과업명</th><th>담당</th><th>기한</th><th>우선순위</th><th>상태</th></tr></thead>
      <tbody>{actions.map(a=><tr key={a.id}>
        <td style={{fontFamily:'IBM Plex Mono',fontSize:'11px',color:statusColor(a.priority)}}>{a.id}</td>
  ...
  return<div>
    ...
    {showNew&&<div className="modal-overlay"onClick={()=>setShowNew(false)}><div className="modal"onClick={e=>e.stopPropagation()}>
      ...
      <div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:16}}>
        ...
        <button className="btn btn-primary"onClick={()=>{onUpdate({...newCoop,id:Date.now(),status:'요청'});setShowNew(false);setNewCoop({title:'',content:'',requester:'',requester_group:'',receiver_group:'',due_date:''});}}>등록</button>
      </div>
    </div></div>}
  </div>;
}
...
export default function App(){
    ...
    (async()=>{
      try{
        ...
        if(!error){
          ...
          // 이슈, 리스크, 액션, 마일스톤 로드
          const{data:dbIss}=await supabase.from('issues').select('*');if(dbIss?.length)setIssues(dbIss);
          const{data:dbRisk}=await supabase.from('risks').select('*');if(dbRisk?.length)setRisks(dbRisk);
          const{data:dbAct}=await supabase.from('action_items').select('*');if(dbAct?.length)setActions(dbAct);
          const{data:dbMs}=await supabase.from('milestones').select('*');if(dbMs?.length)setMilestones(dbMs);
          // 협조 요청 로드
  ...
  const updateCoop=async(c)=>{
    if(c.id&&typeof c.id==='number'&&c.id>100){
      // 새 협조 요청
      setCoops(prev=>[...prev,c]);
      if(supabase&&connected){const{id,...rest}=c;await supabase.from('cooperation_requests').insert(rest);}
    }else{
      setCoops(prev=>prev.map(x=>x.id===c.id?c:x));
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className="content">
          ...
          {page==='issues'&&<IssueList issues={issues}onUpdate={updateIssue}/>}
          {page==='risks'&&<RiskList risks={risks}/>}
          {page==='actions'&&<ActionList actions={actions}onUpdate={updateAction}/>}
